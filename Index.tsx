...
// ── Shared Hero Bar (gradient header for shared-view recipients) ──
const SharedHeroBar = ({
  name,
  kind,
  icon,
  ...
  <header
    className="no-print sticky top-0 z-20 backdrop-blur-sm relative overflow-hidden"
    style={{
      background:
        "linear-gradient(135deg, #0f1f40 0%, #0f6e56 50%, #534AB7 100%)",
  ...
  >
    ...
    <div
      ...
      style={{
        background:
          "radial-gradient(60% 80% at 80% 0%, rgba(255,255,255,0.12), transparent 60%)",
      }}
    />
    <div className="relative flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4">
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        <div className="hidden sm:flex w-8 h-8 rounded-md bg-white/15 items-center justify-center text-white flex-shrink-0">
          {icon}
        </div>
        <div className="min-w-0">
          <div className="text-[10px] font-mono tracking-[0.2em] uppercase text-white/55">
            Shared via PRISM
          </div>
          <div className="text-sm sm:text-base font-semibold text-white truncate">
            {name ? `${name}'s ${kind}` : kind}
          </div>
        </div>
      </div>
      <div className="flex items-center gap-1.5 flex-shrink-0">
        {showDownload && (
          <button
            onClick={onDownload}
            className="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium text-white bg-white/15 hover:bg-white/25 border border-white/20 transition-colors active:scale-[0.98]"
          >
            <Download size={13} />
        ...
        <button
          onClick={onExit}
          className="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium bg-white text-foreground hover:bg-white/90 transition-colors active:scale-[0.98]"
        >
          <ArrowLeft size={13} />
        ...
        <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
          <div
            ...
            style={{
              width: `${pct}%`,
              background:
                "linear-gradient(90deg, #0f1f40 0%, #0f6e56 50%, #534AB7 100%)",
            }}
          />
        </div>
  ...
  return (
    <div className="h-screen bg-background flex flex-col overflow-hidden">
      ...
      <div className="hidden lg:flex flex-col h-full overflow-hidden">
        {/* Fixed header */}
        <div
          className="no-print flex-shrink-0 backdrop-blur z-10 relative"
          style={{
            background:
              "linear-gradient(90deg, hsla(216, 62%, 16%, 0.06) 0%, hsla(166, 75%, 24%, 0.07) 50%, hsla(246, 41%, 51%, 0.06) 100%)",
      ...
      <div className="lg:hidden min-h-screen flex flex-col">
        <header
          className="no-print sticky top-0 z-20 backdrop-blur relative"
          style={{
            background:
              "linear-gradient(90deg, hsla(216, 62%, 16%, 0.06) 0%, hsla(166, 75%, 24%, 0.07) 50%, hsla(246, 41%, 51%, 0.06) 100%)",
            borderBottom: "1px solid hsl(var(--border) / 0.6)",
